...
void _sendMessage() async {
  ...
  // Add "is typing..." message
  final ChatMessage typingMessage = ChatMessage(
    user: ChatUser(id: '2',profileImage: 'https://upload.wikimedia.org/wikipedia/commons/1/13/ChatGPT-Logo.png'),
    createdAt: DateTime.now(),
  ...
  if (_attachedImage != null) {
    ...
    responseText = await _sendImageToAPI(file, text);
    _attachedImage = null;
  } else if (text.isNotEmpty) {
    responseText = await _sendTextToAPI(text);
  } else {
    responseText = '';
  }
...
//   setState(() {
//     _messages.removeWhere((msg) => msg.text == 'chatbot is typing...');
//   });
// }
  ...
  void _addResponseMessage(String responseText) {
    final ChatUser botUser = ChatUser(
      id: '2',
      firstName: 'Gemini',
      profileImage: 'https://upload.wikimedia.org/wikipedia/commons/1/13/ChatGPT-Logo.png'
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Center(child: Text('CHATBOT',style: TextStyle(fontWeight: FontWeight.bold),)),
        actions:[
          IconButton(
              icon: Icon(Icons.add),
